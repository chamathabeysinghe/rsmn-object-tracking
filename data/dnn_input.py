...
def get_rois(path, relative=True):
    frames = []
    df = pd.read_csv(os.path.join(path, 'data.csv'))
    df_data = df.values
    for i in range(len(df.index)):
    ...
    return current_rois, future_rois
...
def get_rois_for_multiple_videos(path, relative=True):
    videos = [os.path.join(path, x) for x in os.listdir(path)]
    result = []
    for video in videos:
        result.append(get_rois(video, relative))
